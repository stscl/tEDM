...
```{r ccm_cvd}
rsp_cvd = tEDM::ccm(cvd,"rsp","cvd",libsizes = seq(32,1032,100),E = 5,k = 6,tau = 10,progressbar = FALSE)
rsp_cvd
...
figa = plot(no2_rsp,ylimits = c(0.5,0.85),ybreaks = seq(0.5,0.85,0.05))
figb = plot(no2_rsp,partial = TRUE,ylimits = c(0.5,0.85),ybreaks = seq(0.5,0.85,0.05))
...
```{mermaid}
graph TD
  rsp[Respirable suspended particulates (rsp)] --> cvd[Cardiovascular diseases (cvd)]
  no2[Nitrogen dioxide (no₂)] --> cvd
  no2 --> rsp
  rsp --> no2
```

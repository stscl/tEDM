...
Employing CCM and PCM to investigate the causal relationships between various air pollutants and cardiovascular diseases:
...
We first use CCM to explore the causal effects of air pollutants on the incidence of cardiovascular diseases.

```{r ccm_cvd, eval=FALSE}
rsp_cvd = tEDM::ccm(cvd,"rsp","cvd",libsizes = seq(32,1032,100),E = 5,k = 6)
rsp_cvd
no2_cvd = tEDM::ccm(cvd,"no2","cvd",libsizes = seq(32,1032,100),E = 5,k = c(8,6))
no2_cvd
so2_cvd = tEDM::ccm(cvd,"so2","cvd",libsizes = seq(32,1032,100),E = 5,k = c(15,6))
so2_cvd
o3_cvd = tEDM::ccm(cvd,"o3","cvd",libsizes = seq(32,1032,100),E = 5,k = 6)
o3_cvd
```

```{r fig1,fig.width=7.5,fig.height=7.5,fig.dpi=100,fig.cap=knitr::asis_output("**Figure 1**. Cross-mapping results between different air pollutants and cardiovascular diseases.")}
...
fig1 = cowplot::plot_grid(figa, figb, figc, figd, ncol = 2,
...
The results shown in Figure 1 indicate the presence of the following causal influences:

- $$\text{rsp} \rightarrow \text{cvd}$$  
- $$\text{no}_2 \rightarrow \text{cvd}$$  
- $$\text{cvd} \rightarrow \text{no}_2$$  
- $$\text{o}_3 \rightarrow \text{cvd}$$


```{r pcm_cvd, eval=FALSE, echo=FALSE}
...
p3 = pcm(cvd,"so2","cvd",c("rsp","no2","o3"),libsizes = seq(10,1300,100),E = 5,k = 6)
...
```{r fig_case2,fig.width=4.5,fig.height=4.5,fig.dpi=100,fig.cap=knitr::asis_output("**Figure 3**. Causal strength scores between annual mean temperature and total annual $\text{CO}_2$ emissions across US counties, with embedding dimension E set to 6 and number of neighbors set to 20.")}

...
When E equals 4, the FNN ratio begins to drop to zero; therefore, we select $E = 4$ as the embedding dimension for the CMC analysis.

```{r covid_cmc,eval=FALSE}
res = names(covid) |> 
  utils::combn(2,simiplify = FALSE) |> 
  purrr::map(\(.l) {
    g = tEDM::cmc(covid,.l[1],.l[2],E = 4, k = 50)
    res = dplyr::mutate(g$xmap,x = .l[1],y = .l[2])
    return(res)
  })
head(res)
